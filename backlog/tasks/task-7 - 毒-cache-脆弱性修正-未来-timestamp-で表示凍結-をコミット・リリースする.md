...
## 実施済み (2026-07-20、working tree 未コミット)

TDD で修正完了。**変更はまだ master の working tree にあり未コミット**。
...
- `bun test` 18 pass / 0 fail 確認済み (GREEN)

## 残作業

1. ~~ブランチを切ってコミット (master 直コミット禁止)~~ 完了: ブランチ
   `fix/poison-cache-and-optin-deploy` のコミット `8058f67` にコミット済み
2. PR → CI → master マージ → release-please のリリース PR マージ (v1.0.6 想定)
